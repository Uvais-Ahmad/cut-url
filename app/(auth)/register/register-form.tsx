    ...
    const onSubmit = async (data: TRegisterFormSchema) => {
        ...
        try {
            ...
            if (response.status === 200) {
                ...
                if (signInResult?.ok) {
                    toast.success('Welcome to Briefly!');
                    router.push('/home');
                    router.refresh();
                } else {
                    toast.warning('Registration successful! Please sign in manually.');
                    router.push('/login');
                }
            } else {
                ...
                if (response.errors) {
                    ...
                    Object.entries(serverErrors).forEach(([field, message]) => {
                        if (field in ['email', 'name', 'password']) {
                            const errorMessage = Array.isArray(message) ? message[0] : message;
                            setError(field as keyof TRegisterFormSchema, { message: errorMessage });
                            toast.error(`${field.charAt(0).toUpperCase() + field.slice(1)}: ${errorMessage}`);
                        }
                    });
                    
                    // Show general error
                    if (serverErrors.email?.includes('already exists')) {
